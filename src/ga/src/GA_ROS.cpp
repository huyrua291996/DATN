  ...
  for (int i = 0; i < width; i++)
    ...
    for (unsigned int iy = 0; iy < costmap_->getSizeInCellsY(); iy++)
    {
      for (unsigned int ix = 0; ix < costmap_->getSizeInCellsX(); ix++)
      {
        //unsigned int cost = static_cast<int>(costmap_->getCost(ix, iy));
        //cout<<cost;
        if ((OGM_data[iy][ix]==0)||(OGM_data[iy][ix] == -1))
          OGM[iy*width+ix]=true;
        else
          OGM[iy*width+ix]=false;
      }
...
//for (uint i=0; i<mapSize; i++)
  ...
  int crossoverType = 3; // 1: one point, 2: two point, 3: modified.
...
/*********************************************************************************/
/*vector<int> GAPlanner::findPath(int startCell, int goalCell, float g_score[])
{
  value++;
  vector<int> bestPath;
...
/*********************************************************************************/
/*vector<int> GAPlanner::constructPath(int startCell, int goalCell,float g_score[])
{
  vector<int> bestPath;
  vector<int> path;
...
/*********************************************************************************/
/*void GAPlanner::addNeighborCellToOpenList(multiset<cells> & OPL, int neighborCell, int goalCell, float g_score[])
{
  cells CP;
